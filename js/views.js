...
export function renderApp(state, route, estimator, notice) {
  ...
  if (!state.configured) {
    return renderSetup(notice);
  }

  if (!state.session) {
    return renderLogin(notice || state.error);
  }
  ...
  return `
    <div class="login-screen">
      <div class="setup-card">
        ...
        <p class="muted-copy">Set the project URL and anon key in <strong>js/config.js</strong>, then run the SQL schema before logging in.</p>
        ${renderNotice(notice)}
      </div>
    </div>
  `;
    ...
    <div class="login-screen">
      <div class="login-grid">
        <section class="login-hero">
          <img class="login-logo" src="./assets/apex-logo.png" alt="Apex Pathway">
          <div class="login-copy">
            <p class="eyebrow">Apex Pathway</p>
            <h1>Automotive build control for serious projects.</h1>
            <p>Track boosted builds, parts, budget, timeline, maintenance, and power estimates from one secure dashboard.</p>
          </div>
        </section>
        <section class="login-card">
          <p class="eyebrow">Secure access</p>
          <h2>Log in</h2>
          <p class="muted-copy">Admin-created accounts only. No public registration.</p>
      ...
      <main class="workspace">
        <header class="topbar">
          ...
          <div class="topbar-actions">
            ...
            <button class="btn ghost" type="button" data-action="logout">
...
function formatGoalPowerMetric(build) {
  const low = Number(build.estimated_hp_low);
  const high = Number(build.estimated_hp_high);
  if (Number.isFinite(low) && Number.isFinite(high) && high >= low) {
    const mid = Math.round((low + high) / 2);
    return {
      value: `${formatNumber(mid)} hp`,
      subtext: `Planning range ${formatNumber(low)}–${formatNumber(high)} hp`
    };
  }

  return { value: "Not set", subtext: "Add estimates on Build Details" };
}
...
function renderDashboard(state, build) {
  ...
  const goalPower = formatGoalPowerMetric(build);
  const upcomingPlanned = state.parts.filter((part) => part.status === "planned").length;
  ...
  const notesBlock = build.customer_brief
    ? `
      <section class="card">
        ...
        <p class="muted-copy">${escapeHtml(build.customer_brief)}</p>
      </section>
    `
    : "";
  ...
  return `
    <div class="content-grid">
      ...
      <section class="grid-4">
        ${renderMetric("Build type", escapeHtml(build.package_name || "—"), "Street, track, or package focus")}
        ${renderMetric("Goal power", goalPower.value, goalPower.subtext)}
        ${renderMetric("Completion", `${budget.percent}%`, "Share of planned part budget in motion (ordered, installed, or completed)")}
        ${renderMetric("Upcoming work", `${upcomingPlanned} item${upcomingPlanned === 1 ? "" : "s"}`, "Parts still in the planned phase")}
      </section>
    ...
    <div class="content-grid">
      ...
      ` : `
        <section class="empty-state">
          ...
          <p>Run <strong>supabase-planner-migration.sql</strong> in Supabase SQL Editor, then refresh this page.</p>
        </section>
      `}
    </div>
  `;
...
function renderNotice(message) {
  ...
  const isError = /error|failed|invalid|required|not|missing|supabase/i.test(message);
